...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        String source = "One of the sights upon which my eyes rest oftenest and with deepest\n" +
        ...
        int numberOfWords = 5, numberOfLines = 4, numberOfParagraphs = 2;

        String[] words = source.split("\\s+");
        //splits the big source string into an array of word-length strings
        //next step: clean up the words from separators (. , - etc)
        for (int i = 0; i < words.length; i++)
        {
            words[i] = words[i].replaceAll("[^\\w]", "");
        }

        //this prints a poem using random elements from the array words[]
        String poem = "\n";
        for (int i = 0; i < numberOfParagraphs; i++) {
            for (int j = 0; j < numberOfLines; j++) {
                for (int k = 0; k < numberOfWords; k++) {
                    ...
                    poem += words[randomWord] + " ";
                }
                poem += "\n";
            }
            poem += "\n";
        }
        testTextView.setText(poem);
    }
}